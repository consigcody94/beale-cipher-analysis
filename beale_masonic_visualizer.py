#!/usr/bin/env python3
"""
BEALE MASONIC VISUALIZER
A tool to visualize potential hidden patterns in the Beale Ciphers using HTML/SVG.
"""
...
def generate_histogram_svg(data, title, width=600, height=300):
    """Generate simple SVG histogram of last digits."""
    last_digits = [n % 10 for n in data]
    ...
    svg = f'<svg width="{width}" height="{height}" style="border:1px solid #ccc; background: white;">'
    svg += f'<text x="{width/2}" y="20" text-anchor="middle" font-family="Arial" font-weight="bold">{title}</text>'
    
    # Bars
    for i in range(10):
        count = counts[i]
        bar_h = (count / max_count) * (height - 60)
        x = start_x + (i * (bar_width + gap))
        y = start_y - bar_h
        
        color = "#e74c3c" if count > (len(data)/10) * 1.5 else "#3498db" # Highlight anomalies
        
        svg += f'<rect x="{x}" y="{y}" width="{bar_width}" height="{bar_h}" fill="{color}" />'
        svg += f'<text x="{x + bar_width/2}" y="{start_y + 15}" text-anchor="middle" font-family="Arial">{i}</text>'
        svg += f'<text x="{x + bar_width/2}" y="{y - 5}" text-anchor="middle" font-family="Arial" font-size="10">{count}</text>'
    
    svg += '</svg>'
    return svg

def generate_scatter_svg(data, title, width=600, height=400):
    """Generate scatter plot of consecutive pairs (n, n+1) to look for geometric patterns."""
    points = []
    max_val = max(data)
    
    # Normalize to plot dimensions
    scale_x = (width - 60) / max_val
    scale_y = (height - 60) / max_val
    
    svg = f'<svg width="{width}" height="{height}" style="border:1px solid #ccc; background: #2c3e50;">'
    svg += f'<text x="{width/2}" y="20" text-anchor="middle" font-family="Arial" fill="white" font-weight="bold">{title}: Consective Pair Scatter</text>'
    
    # Grid lines
    svg += f'<line x1="40" y1="{height-30}" x2="{width-20}" y2="{height-30}" stroke="white" stroke-width="2"/>'
    svg += f'<line x1="40" y1="30" x2="40" y2="{height-30}" stroke="white" stroke-width="2"/>'
    
    for i in range(len(data) - 1):
        x = 40 + (data[i] * scale_x)
        y = (height - 30) - (data[i+1] * scale_y)
        
        # Color based on value magnitude (gradient)
        color_val = int((data[i] / max_val) * 255)
        color = f'rgb(255, {255-color_val}, {color_val})'
        
        svg += f'<circle cx="{x}" cy="{y}" r="3" fill="{color}" opacity="0.6"/>'
        
        # Connect lines to see valid shapes
        if i > 0:
            prev_x = 40 + (data[i-1] * scale_x)
            prev_y = (height - 30) - (data[i] * scale_y)
            svg += f'<line x1="{prev_x}" y1="{prev_y}" x2="{x}" y2="{y}" stroke="{color}" stroke-width="0.5" opacity="0.3"/>'
            
    svg += '</svg>'
    return svg

def main():
    print("Generating Beale Masonic Visualization Report...")
    
    html = """
    <html>
    <head>
        <title>Beale Cipher Masonic Visualizations</title>
        <style>
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; padding: 20px; background: #f5f6fa; }
            .container { max-width: 800px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
            h1, h2 { color: #2c3e50; }
            .chart-box { margin: 30px 0; text-align: center; }
            .desc { color: #7f8c8d; line-height: 1.6; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>🔺 Beale Cipher Masonic Patterns 🔺</h1>
            <p class="desc">
                Visualizing statistical anomalies and hidden geometric formations in the Beale Ciphers using Python-generated SVG charts.
                Red bars in histograms indicate significant anomalies (Last digit bias).
            </p>
            
            <h2>1. Statistical Anomalies: Last Digit Distribution</h2>
            <div class="desc">The 2024 academic research found unnatural biases in the last digits of Ciphers 1 & 3.</div>
    """
    
    # Add Histograms
    html += '<div class="chart-box">'
    html += generate_histogram_svg(CIPHER_1, "Cipher 1: Last Digit Distribution")
    html += "</div>"
    
    html += '<div class="chart-box">'
    html += generate_histogram_svg(CIPHER_2, "Cipher 2 (SOLVED): Last Digit Distribution")
    html += "</div>"
    
    html += '<div class="chart-box">'
    html += generate_histogram_svg(CIPHER_3, "Cipher 3: Last Digit Distribution")
    html += "</div>"
    
    html += """
            <h2>2. Geometric Pattern Search</h2>
            <div class="desc">
                Conspiracies suggest the coordinates form Masonic symbols (Square and Compasses). 
                Here we plot consecutive number pairs (X, Y) to look for ordered structures.
            </div>
    """
    
    # Add Scatter
    html += '<div class="chart-box">'
    html += generate_scatter_svg(CIPHER_1, "Cipher 1 Geometric Plot")
    html += "</div>"
    
    html += """
        <div class="desc">
            <b>Observation:</b> While chaotic, the statistical bias in Cipher 1 is visually distinct from the solved Cipher 2.
            A true "Square and Compass" shape would likely require a purely arbitrary selection of points rather than sequential.
        </div>
        </div>
    </body>
    </html>
    ...
    with open("beale_visuals.html", "w") as f:
        f.write(html)
        
    print(f"Successfully generated 'beale_visuals.html' with {len(html)} bytes.")
